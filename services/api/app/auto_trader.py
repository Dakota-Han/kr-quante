...
class AutoTrader:
    ...
    async def _buy_tick(self, data: Dict[str, Any], daily: Dict[str, Any], day: str, now: datetime) -> Dict[str, Any]:
        ...
        selected = next((decision for decision in strategy.get("selected", []) if decision.get("selected")), None)
        if not selected:
        ...
        decision = StrategyDecision(**selected)
        ...
        quantity = int(budget_krw // limit_price) if limit_price > 0 else 0
        if quantity <= 0:
            data["state"]["last_status"] = "budget_too_small"
            if _flag_once(daily, f"budget_too_small_{decision.code}"):
                _append_event(
                    data,
                    "budget_too_small",
                    "하루 예산이 1주 지정가보다 작아서 자동매수 안 함",
                    level="warning",
                    code=decision.code,
                    side="BUY",
                    quantity=0,
        ...
        preview = OrderPreview(
            ...
            reason="auto Korea overnight lead-lag v3",
            approved=True,
        )
        try:
            ...
            if _flag_once(daily, f"buy_blocked_{decision.code}_{str(exc)}"):
                _append_event(
                    data,
                    "buy_blocked",
                    str(exc),
        ...
        response = await self.client_factory().buy_limit(
            code=preview.code,
            quantity=preview.quantity,
            limit_price=preview.limit_price,
        ...
        daily["buy_order"] = order
        data["state"]["active_position"] = {
            "code": preview.code,
            "name": decision.name,
            "quantity": preview.quantity,
    ...
    async def _sell_tick(self, data: Dict[str, Any], daily: Dict[str, Any], day: str, now: datetime) -> Dict[str, Any]:
        ...
        if daily.get("sell_order") or position.get("exit_order_no"):
        ...
        estimated_pnl = int(round((limit_price - int(position["entry_limit_price"])) * quantity))
        preview = OrderPreview(
            code=code,
            side="SELL",
            quantity=quantity,
        ...
        response = await self.client_factory().sell_limit(
            code=code,
            quantity=quantity,
            limit_price=limit_price,
            account_no=self.account_no,
        ...
        daily["sell_order"] = order
        position["exit_limit_price"] = limit_price
        ...
        return {"status": "submitted", "side": "SELL", "order": order}
